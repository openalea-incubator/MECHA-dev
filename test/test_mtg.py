...
if __name__ == "__main__":
    ...
    init_layer = dict(l_type = [c_type["cortex"], c_type["endodermis"], c_type["stele"]],
                      r_min = [0., 1e-5, 2e-5],
    ...
    for layer_id in g.component_roots_at_scale(segment_id, scale=scales["layer"]):
        ...
        init_cell = dict(c_type = [props["l_type"][layer_id]],
                         minor= [1e-5],
                         major=[1e-5],
                         x_cell = [r_min+5e-6],
                         y_cell = [0.],
                         polygon = [((r_min, 0.), (r_min, 5e-6), (r_min, -5e-6), (r_min+5e-6, 5e-6), (r_min+5e-6, -5e-6), (r_min+1e-5, 0.), (r_min+1e-5, 5e-6), (r_min+1e-5, -5e-6))],
                         vertex_type = [[n_type["wall"], n_type["junction"], n_type["junction"], n_type["wall"], n_type["wall"], n_type["wall"], n_type["junction"], n_type["junction"]]],
                         polygon_adjacency = [((1, 2), (0, 3), (0, 4), (1, 6), (2, 7), (6, 7), (3, 5), (4, 5))])
    ...
    for cell_id in cells:
        ...
        for k, (x, y) in enumerate(props["polygon"][cell_id]):
            init_dict = dict(n_type = props["vertex_type"][cell_id][k],
                         x = x,
                         y = y,
                         adjacent_nodes = [props["polygon"][cell_id][i] for i in props["polygon_adjacency"][cell_id][k]],
                         c_type_a = props["c_type"][cell_id],
                         c_nid_a = cell_nid,
                         c_type_b = -1,
                         c_nid_b = -1,
